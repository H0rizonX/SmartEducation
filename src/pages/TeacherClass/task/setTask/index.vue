<template>
  <div>
    <div class="btn_group" tabindex="-1" aria-hidden="true">
      ...
      <el-button @click="dialogFormVisible = true" type="primary" plain round>
        + 创建任务</el-button
      >
    ...
    <div class="not-data color181E33">暂无暂存任务哦~</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    reflashTime() {
      ...
      const totalSteps = 5; // 生成10个日期项（可以根据需要调整）

      for (let i = 0; i < totalSteps; i++) {
        const date = new Date(currentDate); // 每次循环创建一个新的日期对象
        date.setDate(currentDate.getDate() + i * 4); // 每次步进5天
      ...
      this.timeArr = options; // 更新 options 数据
    },
  },
};
</script>
